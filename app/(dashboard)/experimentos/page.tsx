...
import { TestTube2, Plus, CheckCircle, XCircle, AlertCircle, Sparkles, ChevronDown, ChevronUp, Calendar } from 'lucide-react'
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          messages: [{
            role: 'user',
            content: `Diagnóstico de falha em impressão 3D de alimentos. ${contexto}. Analise as possíveis causas e sugira soluções práticas e ajustes de formulação ou parâmetros. Seja direto e técnico.`,
          }],
        }),
      })
      ...
      setDiagnostico('Erro ao gerar diagnóstico.')
    }
      ...
      <div className="max-w-2xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Registre seus experimentos de impressão e obtenha diagnósticos.</p>
          </div>
          <button
        ...
        {novoAberto && (
          <div className="bg-morphe-dark-2 border border-morphe-orange/20 rounded-xl p-5 mb-6">
                ...
                {(['sucesso', 'parcial', 'falha'] as Resultado[]).map(r => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-3 py-2 rounded-lg border text-sm transition-colors capitalize ${
                        resultado === r ? cfg.bg + ' ' + cfg.color : 'border-border text-muted-foreground hover:border-border/80'
                      }`}
                    >
                      <Icon size={13} />
                      {cfg.label}
              ...
              <div className="mb-4 space-y-3">
                <div>
                  ...
                  <div className="grid grid-cols-1 gap-1.5">
                    {PROBLEMAS_COMUNS.map(p => (
                      <button
                        key={p}
                        onClick={() => setProblemaSelecionado(p === problemaSelecionado ? '' : p)}
                        className={`text-left text-sm px-3 py-2 rounded-md border transition-colors ${
                          problemaSelecionado === p
                            ? 'border-morphe-orange/40 bg-morphe-orange/10 text-morphe-orange'
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Descreva o que observou durante a impressão..."
                    rows={3}
                ...
                <button
                  ...
                  disabled={diagnosticando || (!descricao.trim() && !problemaSelecionado)}
                ...
                >
                  <Sparkles size={13} />
                  {diagnosticando ? 'Gerando diagnóstico...' : 'Gerar diagnóstico com MIA'}
                </button>
            ...
            {resultado === 'sucesso' && (
              <div className="mb-4">
                ...
                <textarea
                  value={descricao}
                  onChange={e => setDescricao(e.target.value)}
                  placeholder="Alguma observação sobre a impressão bem-sucedida?"
                  rows={2}
                  className="w-full bg-morphe-dark border border-border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-morphe-orange/50 resize-none"
                />
              </div>
            ...
            <div className="flex gap-2 pt-2">
              ...
              >
                <TestTube2 size={14} />
                {salvando ? 'Salvando...' : 'Registrar experimento'}
              </button>
              <button
                onClick={() => setNovoAberto(false)}
                className="text-sm text-muted-foreground hover:text-foreground px-4 py-2 border border-border rounded-md transition-colors"
              >
                Cancelar
              </button>
            </div>
        ...
        {experimentos.length === 0 ? (
          <div className="text-center py-16 text-muted-foreground">
            ...
            <button
              ...
              className="text-morphe-orange text-sm hover:underline mt-2 inline-block"
            >
                ...
                <div key={exp.id} className={`bg-morphe-dark-2 border rounded-xl overflow-hidden transition-colors ${cfg.bg}`}>
                  ...
                  {aberto && (
                    <div className="px-4 pb-4 space-y-3 border-t border-border/50 pt-3">
                      ...
                      {exp.diagnostico && (
                        <div>
                          <p className="text-xs text-morphe-orange mb-1">Diagnóstico MIA</p>
