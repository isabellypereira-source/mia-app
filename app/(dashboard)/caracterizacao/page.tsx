...
import { Microscope, Sparkles, ChevronRight, BookOpen } from 'lucide-react'
...
const SECOES: Secao[] = [
  ...
  },
  {
    ...
    protocoloId: 'colapso_filamento',
    campos: [
      { id: 'at_1mm', label: 'Área teórica vão 1 mm', unidade: 'mm²', placeholder: 'ex: 1.20' },
      { id: 'ar_1mm', label: 'Área real vão 1 mm', unidade: 'mm²', placeholder: 'ex: 1.25' },
      { id: 'at_2mm', label: 'Área teórica vão 2 mm', unidade: 'mm²', placeholder: 'ex: 2.40' },
      { id: 'ar_2mm', label: 'Área real vão 2 mm', unidade: 'mm²', placeholder: 'ex: 2.60' },
      { id: 'at_3mm', label: 'Área teórica vão 3 mm', unidade: 'mm²', placeholder: 'ex: 3.60' },
      { id: 'ar_3mm', label: 'Área real vão 3 mm', unidade: 'mm²', placeholder: 'ex: 4.20' },
      { id: 'at_4mm', label: 'Área teórica vão 4 mm', unidade: 'mm²', placeholder: 'ex: 4.80' },
      { id: 'ar_4mm', label: 'Área real vão 4 mm', unidade: 'mm²', placeholder: 'ex: 6.10' },
      { id: 'at_5mm', label: 'Área teórica vão 5 mm', unidade: 'mm²', placeholder: 'ex: 6.00' },
      { id: 'ar_5mm', label: 'Área real vão 5 mm', unidade: 'mm²', placeholder: 'ex: 8.50' },
      { id: 'at_6mm', label: 'Área teórica vão 6 mm', unidade: 'mm²', placeholder: 'ex: 7.20' },
      { id: 'ar_6mm', label: 'Área real vão 6 mm', unidade: 'mm²', placeholder: 'ex: 11.40' },
    ],
  },
  {
    id: 'tpa_cooking_loss',
    titulo: 'TPA + Perda de Massa no Cozimento',
    protocoloId: 'tpa_cooking_loss',
    campos: [
  ...
  },
  {
    ...
    protocoloId: 'fidelidade_dimensional',
    campos: [
  ...
  },
  {
    ...
    protocoloId: 'precisao_impressao',
    campos: [
      ...
      { id: 'altura_max_camadas', label: 'Camadas até colapso', unidade: 'camadas', placeholder: 'ex: 38' },
    ],
  },
]
...
function calcularResultados(secaoId: SecaoId, v: Record<string, string>): string {
  ...
  if (secaoId === 'colapso_filamento') {
    ...
    const detalhes = vaos.map(vao => {
      const at = num(`at_${vao}`), ar = num(`ar_${vao}`)
      return ar > 0 ? `${vao}: Cf = ${fmt((at / ar) * 100, 1)}%` : null
    }).filter(Boolean).join(' | ')

    let interp: string
    if (media < 20) interp = 'colapso total — material muito fluido. Adicionar agentes estruturantes (hidrocolóides, amidos, proteínas).'
    else if (media < 70) interp = 'sustentação parcial — estrutura moderadamente estável. Considerar ajuste fino.'
    else interp = 'ponte estável — material ideal para impressão 3D.'
  ...
  if (secaoId === 'tpa_cooking_loss') {
    ...
    if (m0 > 0 && ma >= 0) {
      ...
      let interpCL: string
      if (cl < 5) interpCL = 'baixa perda — alta estabilidade e retenção de água'
      else if (cl < 15) interpCL = 'perda moderada'
      else interpCL = 'alta perda — estrutura instável'
  ...
  if (secaoId === 'sinerese') {
    ...
    if (w0 > 0 && wa >= 0) {
      ...
      let interp: string
      if (syn < 5) interp = 'baixa sinérese — estrutura estável, boa interação proteínas-hidrocolóides'
      else if (syn < 15) interp = 'sinérese moderada — estabilidade parcial'
      else interp = 'alta sinérese — estrutura instável. Considerar mucilagem, goma guar ou xantana'
  ...
  if (secaoId === 'fidelidade_dimensional') {
    ...
    if (am > 0 && an > 0) {
      const fd = (am / an) * 100
      let interp: string
      if (fd >= 95 && fd <= 105) interp = 'alta precisão dimensional'
      else if (fd > 105) interp = 'over-extrusion. Reduzir vazão ou aumentar velocidade'
      else interp = 'subextrusão ou colapso. Aumentar vazão ou reduzir velocidade'
  ...
  if (secaoId === 'precisao_impressao') {
    ...
    if (l1 > 0 && h1 > 0 && h3 > 0 && l2 > 0 && h2 > 0) {
      ...
      else if (pa >= 75) interp = 'precisão aceitável com pequenos desvios'
      else interp = 'desvios significativos — possível colapso parcial ou inconsistência'
      partes.push(`Precisão de Impressão (PA) = ${fmt(pa, 1)}% → ${interp}`)
    }
    if (camadas > 0) {
      let interpA: string
      if (camadas >= 30) interpA = 'excelente capacidade de sustentação vertical'
    ...
    <div className="h-full overflow-y-auto" style={{ background: '#fff8f1' }}>
      <div className="section-alt border-b border-[#e5d9c1] px-8 py-6 mb-6">
        ...
        <p className="text-sm text-[#58413c] mt-1">
          Insira valores experimentais e calcule índices de printabilidade, textura e estabilidade.
        </p>
      </div>
              ...
              <div key={secao.id} className="bg-white rounded-2xl shadow-tonal overflow-hidden">
                ...
                {aberta && (
                  <div className="px-4 pb-4 border-t border-[#e5d9c1] pt-4">
                    {secao.protocoloId && (
                      <Link
                        href={`/protocolos/${secao.protocoloId}`}
                        className="inline-flex items-center gap-1.5 text-[10px] text-[#003223] hover:underline mb-4"
                      >
                        <BookOpen size={11} /> Ver protocolo completo →
                      </Link>
                    )}
